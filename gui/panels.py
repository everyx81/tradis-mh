...
class FileManagerWidget(QWidget):
    ...
    def _setup_settings_tab(self):
        ...
        self.btn_show_manual = NeonButton("정산 매뉴얼", color="cyan")
    ...
    def _show_manual_dialog(self):
        ...
        dlg.resize(700, 800)
        dlg.setStyleSheet(f"background-color: {CT['bg_1']}; color: {CT['fg_1']};")

        layout = QVBoxLayout(dlg)
        layout.setContentsMargins(20, 20, 20, 20)

        # 타이틀
        title = QLabel("TRADIS 정산 매뉴얼")
        title.setStyleSheet(f"font-size: 15pt; font-weight: 600; color: {CT['fg_0']}; margin-bottom: 12px;")
        title.setAlignment(Qt.AlignmentFlag.AlignCenter)
        layout.addWidget(title)
        ...
        # 매뉴얼 내용 (코드 내장) — 정산 패널 전용: 파일이 들어와서 서버로
        # 나가기까지의 전체 흐름과 각 단계에서 쓰는 기능을 안내
        manual_content = """
# TRADIS 정산 매뉴얼

수입 통관 서류(PDF)가 **들어와서 → 분석되고 → 묶여서 → 서버로 나가기**까지,
정산 탭에서 일어나는 전체 흐름을 순서대로 설명합니다.

---

## 전체 흐름 한눈에 보기

**① 준비** → **② 파일 투입** → **③ AI 분석·이름변경** → **④ 그룹 카드 확인**
→ **⑤ 매핑 확인·수정** → **⑥ 합치기 / 폴더 정리** → **⑦ 선적서류 첨부** → **⑧ 서버로 이동**
...
| 항목 | 어디서 | 없으면 |
|------|--------|--------|
| **Gemini API 키** | 좌측 사이드바 톱니바퀴 | AI 분석 불가 — 파일이 처리되지 않음 |
| **감시 폴더** | 좌측 사이드바 TARGET DIRECTORY | 파일을 투입할 곳이 없음 |
| **수입/수출 서버 경로** | 설정 탭 | 마지막 단계(서버 이동) 불가 |
| **선적서류폴더** | 설정 탭 | 선적서류 자동 검색 불가 |

준비가 끝나면 좌측의 **T 로고(시작)** 를 눌러 감시를 켭니다.

---

## ② 파일 투입

감시 폴더에 통관 서류 PDF를 넣습니다 (드래그, 복사, 스캔 저장 모두 가능).
파일이 들어오는 즉시 다음 단계가 자동으로 시작됩니다.

---

## ③ AI 분석 → 자동 이름변경 (자동)

각 PDF를 AI가 읽고 **서류 종류 / 업체명 / B/L번호 / 금액**을 추출한 뒤,
파일명을 규칙에 맞게 자동으로 바꿉니다.

- 예: `스캔_0001.pdf` → `한의코퍼레이션(GOSUSHH32180942)자금정산서.pdf`
- 한 번 분석한 파일은 캐시에 저장되어 다시 넣어도 즉시 처리됩니다.
- 파일명 규칙은 설정 탭의 **커스텀 네이밍**에서 바꿀 수 있습니다.

---

## ④ 그룹 카드 확인

같은 **B/L 번호**의 서류끼리 자동으로 묶여 **그룹 카드**가 만들어집니다.
카드에는 자금정산서를 기준으로 필요한 서류 체크리스트가 표시됩니다.

| 카드 버튼 | 기능 |
|------|------|
| **AI 분석** | 이 그룹만 다시 분석 (잘못 읽었을 때) |
| **▲ ▼** | 병합 순서 변경 |
| **+ 행 추가** | 목록에 없는 파일을 수동으로 추가 |
| **파일 추가** | 선적서류 등 외부 파일 첨부 |
| **돋보기** | 해당 PDF를 기본 뷰어(어도비)로 바로 열기 |

---

## ⑤ 매핑 확인·수정

정산서의 **비용 항목**과 실제 **파일**이 자동으로 짝지어집니다(매핑).

**자동 매칭 규칙:**
- 병합 순서: 자금정산서 → 수입신고필증 → 납부고지서 → 세금계산서 순
- 비용 항목명 ↔ 파일명 키워드 매칭, 실패 시 금액으로 보정
- 통합 계산서는 내부 청구 항목까지 분석해서 배정
- 식물검역/식품검역/CITES 등 요건 서류도 자동 매칭

잘못 짝지어진 항목은 드롭다운으로 직접 바꿀 수 있고,
한 번 고친 짝은 기억되어 다음부터 자동 적용됩니다.
...
| 버튼 | 하는 일 | 언제 사용 |
|------|---------|----------|
| **합치기 실행** | 정산서 순서대로 하나의 PDF로 병합 + 폴더 정리 | 일반적인 경우 |
| **폴더 정리** | 병합 없이 B/L 폴더만 만들어 파일 이동 | 병합이 필요 없는 건 |
| **되돌리기** | 합치기/정리 이전 상태로 복원 | 실수했을 때 (정리 기록에서) |

---

## ⑦ 선적서류 자동 검색·첨부

합치기가 끝나면 **선적서류폴더**에서 B/L·Invoice 번호로 맞는 서류를
자동 검색해 팝업으로 보여줍니다. 선택하면 B/L 폴더 안으로 이동됩니다.

- 자동으로 못 찾은 경우: 카드의 **파일 추가** 버튼으로 직접 첨부

---

## ⑧ 서버로 이동 (마무리)

우측 **파일 매니저**에서 완성된 B/L 폴더를 서버로 보냅니다.

| 기능 | 설명 |
|------|------|
| **MOVE TARGET** | 감시 폴더 안의 완성된 폴더 목록 |
| **→ 수입 / → 수출** | 선택한 폴더를 해당 서버 경로로 이동 |
| **TRADIS SEARCH** | 전체 드라이브에서 파일 검색 (Everything 기반) |
| **파일 탐색기** | 폴더 트리에서 직접 탐색·드래그 이동 |

여기까지 완료되면 한 건의 정산 처리가 끝납니다.
...
| 증상 | 해결 |
|------|------|
| 파일이 처리되지 않음 | API 키 확인 → 감시가 켜져 있는지(T 로고) 확인 |
| 서류 정보를 잘못 읽음 | 카드의 **AI 분석**으로 재분석 |
| 매핑이 틀림 | 드롭다운으로 수정 (다음부터 기억됨) |
| 합치기를 잘못함 | 정리 기록에서 **되돌리기** |
"""
